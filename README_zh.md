...
4. **依赖权衡 (pi-mono 锁定)**：本项目底层高度依赖 `@mariozechner/pi-mono` 提供的模型抽象。这是一个经过深思熟虑的 Trade-off：利用其“小而美”的特性实现快速起步与厂商解耦。尽管存在单点依赖风险（如项目停更），但本引擎的**核心业务逻辑（官僚制路由流转、Prompt 体系、TypeBox 状态机）其实是与底座完全解耦的**。未来若遭遇天花板，仅需重写 `orchestrator.ts` 中的薄层 Adapter（约300行代码），即可极低成本迁移至 Vercel AI SDK 或其他通用框架。
